...
- **Parches locales** (razón única del vendoring — no existen upstream):
  ...
  2. `src/codec.rs`: límites a parámetros controlados por el atacante — iteraciones del MAC en `verify_mac` (máx. 1.000.000, finding R4-001 de review-t077b-scope-recovery-20260715-01), iteraciones PBES1 y PBES2/PBKDF2 en `decrypt` (máx. 1.000.000) y coste scrypt `N` (máx. 2^20). Evita DoS de CPU/memoria con contenedores PFX maliciosos.
