...
/// `cancelled_status` is the operation's own word for backing out, and
/// `proceed` is what it does when the user accepts — the prompt itself is the
/// same question for an ordinary save and for applying protection, because
/// both reach the rewriter and the rewriter is what breaks the signature.
fn confirm_signature_loss(
    window: &ApplicationWindow,
    viewer: &Viewer,
    cancelled_status: &'static str,
    proceed: Rc<dyn Fn()>,
) {
    let dialog = AlertDialog::builder()
    ...
    dialog.choose(Some(window), None::<&gio::Cancellable>, {
        ...
        move |response| {
            if response == Ok(1) {
                ...
                viewer.status.set_text(cancelled_status);
            }
        }
    });
}
...
fn save_current_to(
    ...
    if breaks_signature {
        ...
        confirm_signature_loss(window, viewer, "Save cancelled.", {
            let viewer = viewer.clone();
            Rc::new(move || {
                spawn_save(
                    &viewer,
...
/// [`spawn_protect`] — the protecting twin of [`SignRequest`], bundled for the
/// same reason: the same handful of values would otherwise repeat
/// field-by-field across every step of the warn → choose → confirm →
/// background-save chain.
#[derive(Clone)]
pub(crate) struct ProtectRequest {
    pub(crate) token: super::state::SessionToken,
    pub(crate) document: Document,
    pub(crate) backing: super::state::SaveBacking,
...
/// The Protect twin of [`begin_sign`]: warn if this rewrite would invalidate a
/// signature, ask where to write, then protect on a worker thread.
///
...
pub(crate) fn begin_protect(window: &ApplicationWindow, viewer: &Viewer, request: ProtectRequest) {
    if protect_breaks_signature(&request) {
        confirm_signature_loss(window, viewer, "Protection cancelled.", {
            let window = window.clone();
            let viewer = viewer.clone();
            Rc::new(move || {
...
/// [`begin_sign`]'s chooser, for protection. A protected document is written
/// to a destination the user names, like every other write this shell
/// performs — there is no "the current file" to silently overwrite.
...
/// [`confirm_sign_destination`]'s twin: the same "Replace existing PDF?"
/// guard, ending in [`spawn_protect`].
fn confirm_protect_destination(
    window: &ApplicationWindow,
    viewer: &Viewer,
    request: ProtectRequest,
    destination: PathBuf,
    ...
    match destination.try_exists() {
        Ok(false) => spawn_protect(viewer, request, destination, signatures),
        Err(error) => viewer.status.set_text(&format!(
            "Could not check whether {} already exists: {error}",
            destination.display()
        )),
        Ok(true) => {
            ...
            dialog.choose(Some(window), None::<&gio::Cancellable>, {
                ...
                move |response| {
                    if response == Ok(1) {
                        spawn_protect(&viewer, request.clone(), destination.clone(), signatures);
                    } else {
                        viewer.status.set_text("Protection cancelled.");
                    }
                }
...
/// into the session — the protecting twin of [`spawn_save`]/[`spawn_sign`].
fn spawn_protect(
    viewer: &Viewer,
    request: ProtectRequest,
    destination: PathBuf,
    ...
    viewer.status.set_text("Protecting PDF...");
    glib::spawn_future_local({
        let viewer = viewer.clone();
        async move {
            let token = request.token;
            ...
            match result {
                Ok(reopened) if let Some(generation) = prepare_reopened_session(&viewer, token) => {
                    show_document(&viewer, generation, reopened);
                    viewer.status.set_text("PDF protected, saved and reopened.");
                }
                Ok(reopened) => close_document_in_background(reopened.document),
                Err(error) if session_matches(&viewer, token) => viewer
                    .status
                    .set_text(&format!("Could not protect PDF: {error}")),
                Err(_) => {}
            }
        }
    });
...
/// [`save_snapshot_and_reopen`]'s protecting twin: the same serialize →
...
fn protect_snapshot_and_reopen(
    ...
    let renderer = PdfiumRenderer::new();
    let handle = renderer
        .open_document_from_bytes(bytes.clone(), Some(&open_password))
        .map_err(|error| error.to_string())?;
    let page_count = renderer
        .page_count(handle, Priority::Visible)
        .wait()
...
mod tests {
    ...
    fn protecting_a_document_makes_the_new_password_the_only_one_that_opens_it() {
        ...
        fs::write(&source, super::SAMPLE_PDF).expect("seed the unprotected sample");
        ...
        let _ = super::PdfiumRenderer::new().close_document(reopened.document);
